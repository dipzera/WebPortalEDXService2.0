...
export const LoginPage = () => {
  ...
  const onSubmit = (values) => {
    ...
    const requestOptions = {
      method: "POST",
      headers: { "Content-Type": "application/json", "Authorization": "Basic asd:qwe" },
      body: JSON.stringify({ Name: values.email, Password: values.password }),
    };



    fetch("https://api.edi.md/WebPortalEDXService/json/Logon", requestOptions)
      ...
      .then((loginData) => {
        if (loginData.ErrorCode === 0) {
          ...
          /* HANDLE ERROR (INCORRECT LOGIN/PASS) */
          setError(true);
        }
      })
                        ...
                        <Form className="form mt-5 contact-form">
                          {error && (
                            <TextAlert error={true}>
                              Adresa de email sau parola este incorecta!
                            </TextAlert>
                          )}
